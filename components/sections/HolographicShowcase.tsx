...
const ChatbotVisual = () => {
  ...
  useEffect(() => {
    ...
    }, 4000); // Slower message appearance
  ...
  return (
    <div className="w-full h-full p-4 flex flex-col gap-3 relative overflow-hidden">
      ...
      {messages.map((m, i) => (
        <div 
          key={m}
          className={`bg-cyan-900/40 border border-cyan-500/30 rounded-2xl p-3 max-w-[80%] backdrop-blur-sm animate-float-sm ${i % 2 === 0 ? 'self-start rounded-tl-none' : 'self-end rounded-tr-none border-cyan-400/50 bg-cyan-800/50'}`}
          style={{ animationDelay: `${i * 0.5}s`, transitionDuration: '1s' }}
        >
          <div className="flex gap-2 items-center mb-1">
            <div className={`w-2 h-2 rounded-full ${i % 2 === 0 ? 'bg-cyan-400' : 'bg-white'} animate-pulse`} />
      ...
      <div className="absolute bottom-4 left-4 flex gap-1 bg-black/40 p-2 rounded-full border border-white/10">
        ...
        <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '200ms' }} />
        <span className="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-bounce" style={{ animationDelay: '400ms' }} />
      </div>
    </div>
  );
};
...
const AnalyticsVisual = () => {
  ...
  return (
    <div className="w-full h-full p-6 relative flex flex-col justify-end overflow-hidden">
       <div className="absolute top-4 right-4 text-emerald-400 font-mono text-xl font-bold tracking-widest">
       ...
       <div className="flex items-end justify-between gap-2 h-32">
         {[40, 65, 30, 80, 55, 90, 45].map((h, i) => (
           <div 
            key={i} 
            className="w-full bg-emerald-500/20 border-t-2 border-emerald-400 rounded-t-sm relative group"
            style={{ height: `${h}%` }}
           >
             <div className="absolute bottom-0 w-full bg-emerald-500/10 h-full animate-pulse" style={{ animationDelay: `${i * 100}ms` }} />
           </div>
         ))}
       </div>
    </div>
...
const VoiceVisual = () => {
  return (
    <div className="w-full h-full flex items-center justify-center gap-1.5 relative overflow-hidden bg-orange-900/10">
       <div className="absolute inset-0 bg-gradient-to-t from-orange-500/10 via-transparent to-transparent" />
       {[...Array(9)].map((_, i) => (
         <div 
           key={i}
           className="w-3 bg-gradient-to-t from-orange-600 to-orange-400 rounded-full animate-wave-stretch shadow-[0_0_15px_rgba(249,115,22,0.4)]"
           style={{ 
             height: '20%', 
             animationDelay: `${i * 0.2}s`, // Slower stagger
             animationDuration: '3s' // Slower duration
           }}
         />
       ))}
    </div>
...
// --- Main Holographic Card Component (Performance Optimized) ---

const HoloCard: React.FC<{ product: ProductCard }> = ({ product }) => {
  const cardRef = useRef<HTMLDivElement>(null);
  const glowRef = useRef<HTMLDivElement>(null);
  
  // Use Refs for interaction to avoid React re-renders on every mouse move
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const rotateY = ((x - centerX) / centerX) * 15; // Reduced rotation for subtlety
  ...
  return (
    ...
    >
      ...
      >
        {/* Animated Rotating Rainbow Border */}
        ...
        <div className="absolute inset-0 rounded-[30px] overflow-hidden bg-[#13131a]/80 backdrop-blur-xl border border-white/10 shadow-2xl z-10 flex flex-col">
          ...
          {/* Holographic Sheen Overlay (Direct Ref Control) */}
          <div 
            ref={glowRef}
...
export const HolographicShowcase: React.FC = () => {
  ...
  // Auto-Scroll Logic - Slower interval
  useEffect(() => {
    ...
    }, 6000); // 6 seconds per scroll (Slower)
